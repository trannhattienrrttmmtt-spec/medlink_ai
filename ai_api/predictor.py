...
class AMDGTPredictor:
    ...
    def _precompute_all(self):
        """Tính trước score cho TẤT CẢ cặp drug-disease → predict instant"""
        import time

        # Kiểm tra file cache đã có chưa
        cache_file = os.path.join(self.amdgt_dir, "Result", self.dataset, "score_matrix_current.npy")
        ...
        print(f"[{self.dataset}] Computing score matrix (first time only)...")
        start = time.time()
        ...
        # Lưu ra file để lần sau load nhanh
    ...
    def predict(self, input_type, keyword, top_k=5, fold_index=0):
        ...
        if input_type == "drug":
            ...
            # Lấy scores từ pre-computed matrix
